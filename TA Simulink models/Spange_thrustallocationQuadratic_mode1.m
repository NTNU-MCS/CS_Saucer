function [sys,ThrusterPos,str,ts] = Spange_thrustallocationQuadratic_mode1(t,x,u,flag,u_e,Thruster,C) 
...
sizes.NumDiscStates  = 2;   % Number of discrete states in the system 
sizes.NumOutputs     = 2;   % Number of outputs (Output of each angle of each thruster and desired thrust of each thruster)
sizes.NumInputs      = 5;  % Number of inputs (Input: Desired angle and Desired thrust vector)
...
K = ones(1,2); % Resets K For each time iteration
...
Beq = tc; 
ub =  [1.5 1.5 0.01 0.01 0.01]'; %[Max_thrust max_slack]
lb = -[1.2 1.2 0.01 0.01 0.01]';
X0 = u(5);
...
u_d = sqrt(X(1)^2+X(2)^2);      % Optimized Thrust
alpha1 = atan2d(X(2),X(1));
...
Desired_Alpha = alpha1(1)';
Desired_thrust = u_d(1);
