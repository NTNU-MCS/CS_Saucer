...
x0 = [Thruster.T1(3) Thruster.T3(3) 0 0]';
...
C3 = C(2,:);    % Angle constraints Thruster 2

K = ones(1,3); % Resets K For each time iteration
...
Beq = tc; 
ub =  [1.5 1.5 0.2 0.01 0.01 0.01]'; %[Max_thrust max_slack]
lb = -[1.2 1.2 0.2 0.01 0.01 0.01]';
X0 = [u(6),u(7)]';
...
u_d = [sqrt(X(1)^2+X(2)^2) X(3)] ;      % Optimized Thrust
alpha1 = [atan2d(X(2),X(1)) -90];
