function [sys,ThrusterPos,str,ts] = Spange_thrustallocationQuadratic_mode4(t,x,u,flag,u_e,Thruster,C) 
...
sizes.NumDiscStates  = 6;   % Number of discrete states in the system 
sizes.NumOutputs     = 6;   % Number of outputs (Output of each angle of each thruster and desired thrust of each thruster)
sizes.NumInputs      = 9;  % Number of inputs (Input: Desired angle and Desired thrust vector)
...
x0 = [Thruster.T1(3) Thruster.T2(3) Thruster.T3(3)  0 0 0]';
...
K = ones(1,5); % Resets K For each time iteration
...
W = eye(5);
W(5,5) = 0.01;               %Do not use bowthruster that much
q = sum(W);
...
Beq = tc; 
ub =  [1.5 1.5 1.5 1.5 0.2 0.01 0.01 0.01]'; %[Max_thrust max_slack]
lb = -[1.2 1.2 1.2 1.2 0.2 0.01 0.01 0.01]';
% ub =  [1.5*cosd(alpha(1)) 1.5*sind(alpha(1)) 1.5*cosd(alpha(2)) 1.5*sind(alpha(2)) 0.6 0.01 0.01 0.01]'; %[Max_thrust max_slack]
% lb = -[1.5*cosd(alpha(1)) 1.5*sind(alpha(1)) 1.5*cosd(alpha(2)) 1.5*sind(alpha(2)) 0.6 0.01 0.01 0.01]';
X0 = [u(7),u(8),u(9)]';
...
u_d = [sqrt(X(1)^2+X(2)^2) sqrt(X(3)^2+X(4)^2) X(5)] ;      % Optimized Thrust
alpha1 = [atan2d(X(2),X(1)) atan2d(X(4),X(3)) -90];
...
Desired_Alpha = [alpha1(1),alpha1(2),alpha1(3)]';
Desired_thrust = [u_d(1) u_d(2) u_d(3)]';
