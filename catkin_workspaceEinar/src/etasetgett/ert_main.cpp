...
{
  runModel = (rtmGetErrorStatus(etasetgett_M) == (NULL));
  while (runModel) {
...
{
  rtmSetErrorStatus(etasetgett_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(etasetgett_M, 0);

  // Initialize model
  etasetgett_initialize();
