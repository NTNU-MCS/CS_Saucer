...
{
  runModel = (rtmGetErrorStatus(LidarMapSim_M) == (NULL));
  while (runModel) {
...
{
  rtmSetErrorStatus(LidarMapSim_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(LidarMapSim_M, 0);

  // Initialize model
  LidarMapSim_initialize();
