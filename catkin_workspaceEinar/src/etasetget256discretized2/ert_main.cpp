...
{
  runModel = (rtmGetErrorStatus(etasetget256Discretized2_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    etasetget256Discretized2_step();
...
{
  rtmSetErrorStatus(etasetget256Discretized2_M, "stopping the model");
}
...
void terminateTask(void *arg)
{
  ...
  // Terminate model
  etasetget256Discretized2_terminate();
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(etasetget256Discretized2_M, 0);

  // Initialize model
  etasetget256Discretized2_initialize();
