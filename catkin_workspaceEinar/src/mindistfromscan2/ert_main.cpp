...
{
  runModel = (rtmGetErrorStatus(mindistfromscan2_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    mindistfromscan2_step();
...
{
  rtmSetErrorStatus(mindistfromscan2_M, "stopping the model");
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(mindistfromscan2_M, 0);

  // Initialize model
  mindistfromscan2_initialize();
