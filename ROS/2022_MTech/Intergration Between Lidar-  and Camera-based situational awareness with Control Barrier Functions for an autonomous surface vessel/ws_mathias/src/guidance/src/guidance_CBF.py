...
from messages.msg import s, state_estimation, guidance_signals
...
class guidance_CBF(object):
    ...
    def __init__(self):
        ...
        self.obstacles = None            # No obstacles at the beginning
        self.r_o = 1
...
if __name__ == '__main__':
    ...
    # p0 = np.array([[0], [0]])
    # pt = np.array([[6], [0]])
    obstacle1 = Obstacle(0.132, -0.532, 0)
    obstacle3 = Obstacle(1.54, 1.532, 0)
    obstacle2 = Obstacle(40, 0.5, 0)
    obstacles = [obstacle2]
    ...
    while not rospy.is_shutdown():
        ...
        if guidance.flag:
            ...
            if guidance.s[0] > 0.1 and len(guidance.obstacles) < 2:
                obstacles.append(obstacle1)
            if guidance.s[0] > 0.3 and len(guidance.obstacles) < 3:
                obstacles.append(obstacle3)
            guidance.obstacle_callback(np.array(obstacles))
