...
from common_tools.math_tools import R2, rad2pipi, string2array, ssa
from common_tools.lib import qualisys, Obstacle
...
class ObstacleConverter(object):
    ...
    def __init__(self):
        ...
        self.t_cb = [[0.06], [0.0], [-0.075]]                           # Translation from camera to center of origin (CO) in BODY [m]
        ...
        measure_sub = message_filters.Subscriber("fusion/boat", FusionMessage)
        ts = messafe_filters.TimeSynchronizer([heading_sub, measure_sub], 10) # Message filtes ensure that the detected obstacles and the heading measurement have the same timestamp, as they run at different frequencies.
    ...
    def callback(self, data):
        ...
        obstacles_cam = convert2Cam(obstacles)                             # Convert to camera frame of referenc    e
        obstacles_body = convert2BODY(obstacles_cam)                       # Convert to BODY frame
        obstacles_NED = convert2Ned(obstacles_body)                        # Convert to NED
    ...
    def convert2BODY(self, obstacle_poses):
        for obstacle in obstacles:
