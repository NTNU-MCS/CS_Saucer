...
{
  runModel = (rtmGetErrorStatus(MotionControlCompile_M) == (NULL)) &&
    !rtmGetStopRequested(MotionControlCompile_M);
  while (runModel) {
...
{
  rtmSetErrorStatus(MotionControlCompile_M, "stopping the model");
}
...
void terminateTask(void *arg)
{
  ...
  // Terminate model
  MotionControlCompile_terminate();
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(MotionControlCompile_M, 0);

  // Initialize model
  MotionControlCompile_initialize();
